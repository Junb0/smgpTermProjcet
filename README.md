...
- Cat
  ...
  - 주요 속성 : 고양이 종류, 별 개수, 자신이 위치한 슬롯의 인덱스
  - 일정시간마다 선두의 적에게 투사체를 발사한다. (구현중)
  - 터치되었을 경우 자신의 속성을 물려받는 DraggedCat 오브젝트를 생성 한다.
  - DraggedCat에 의해 합성이 성공할 경우 등급이 오르며 랜덤으로 타입을 변경 한다.

- DraggedCat

  ![드래그고양이](https://github.com/Junb0/smgpTermProjcet/blob/main/docs/draggedCat.png)
  - 주요 속성 : Cat과 동일
  - 플레이어가 터치중인 지점으로 따라간다.
  - 터치가 종료되었을 경우 삭제된다. 이 때, 드롭 된 위치에 따라 해당 위치의 고양이와 합성 되거나 원래의 Cat 객체를 생성한다.

- CatSpawner
  - 주요 속성 : 소환 가격, 비어있는 슬롯 정보
...
- Enemy
  ...
  - EndPoint까지 살아서 도달 시 플레이어의 체력을 10 감소시킨다.
  - 고양이의 투사체에 충돌 시 HP가 감소하며 HP가 0 이하가 되면 삭제되며 플레이어의 골드를 증가시킨다.
...
- UpgradeManager
  - 주요 속성 : 고양이 타입에 따른 업그레이드 현황 배열
  - 업그레이드 버튼 터치 시 해당 고양이 타입의 업그레이드 카운트를 증가시킨다.
  - 각 고양이 객체의 최종 데미지 산출 시 참조된다.
